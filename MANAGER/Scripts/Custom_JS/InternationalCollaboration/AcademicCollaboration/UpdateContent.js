...
function getLTContent() {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data != null) {
                if (data.success) {
                    $('#lt_describe_content_description').val(data.obj.description);
                } else {
...
$('#lt_describe_content_save').on('click', function () {
    $.ajax({
        url: "/AcademicCollaboration/updateLTContent",
        type: "POST",
        cache: false,
        data: {
            collab_type_id: 2, //dài hạn
            language_id: function () { return $('#lt_describe_content_language').val(); },
            description: function () { return $('#lt_describe_content_description').val(); }
        },
        dataType: "json",
        success: function (data) {
            if (data != null) {
